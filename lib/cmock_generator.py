...
class CMockGenerator:
    ...
    def _write_mock_header_content(self, file, mock_project):
        ...
        content = [
            ...
            f"#ifndef _{define_name}_H\n",
            f"#define _{define_name}_H\n\n",
            f"#include \"{self.framework}.h\"\n",
        ]

        content.extend([f"#include {inc}\n" for inc in self.includes_h_pre_orig_header])
        content.append(f"{self.config.options['orig_header_include_fmt'] % orig_filename}\n")
        content.extend([f"#include {inc}\n" for inc in self.includes_h_post_orig_header])

        plugin_includes = self.plugins.run("include_files")
        if plugin_includes:
            content.append(plugin_includes)
